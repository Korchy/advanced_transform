...
def UserPresets(self, context, chen):
    ...
    if chen is True:
        save_user_drag = context.preferences.inputs.use_drag_immediately
        if save_user_drag is not True:
            context.preferences.inputs.use_drag_immediately = True

    elif chen is False:
        context.preferences.inputs.use_drag_immediately = save_user_drag
...
def CreateOrientation(self, context):
    ...
    if temp_name != 'GLOBAL':
        ...
        # context.window.scene.transform_orientation_slots[0].type = temp_name
...
def GetUserSnap(self, context):
    ...
    user_snap_element = context.scene.tool_settings.snap_elements
...
def SetUserSnap(self, context):
    context.scene.tool_settings.snap_elements = user_snap_element
...
def draw_callback_px(self, context):
    ...
    coords = []
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, -1.0, 0.0)))))
    indices = ((0, 1, 2), (2, 0, 3))
    ...
    shader.bind()
    shader.uniform_float("color", (0.471938, 0.530946, 0.8, 0.06))
    batch.draw(shader)

    bgl.glLineWidth(2.0)
    coords = []
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.8, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.8, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.6, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.6, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.4, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.4, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.2, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.2, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.0, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((0.0, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-0.2, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-0.2, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-0.4, 1.0, 0.0)))))
    ...
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-0.8, 1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-0.8, -1.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 0.8, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 0.8, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 0.6, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 0.6, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 0.4, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 0.4, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 0.2, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 0.2, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, 0.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, 0.0, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, -0.2, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((-1.0, -0.2, 0.0)))))
    coords.append(mw @ ((Zoom(self, context) / zoom_scale) * (scale * Vector((1.0, -0.4, 0.0)))))
...
def draw_callback_rot(self, context):
    ...
    point3 = None
    bgl.glEnable(bgl.GL_BLEND)
    if self.temp_loc_first is not None:
        ...
        if self.exc_axis == 'x':
            color = (0.8, 0.245542, 0.210406, 0.2)
        elif self.exc_axis == 'y':
            color = (0.265925, 0.8, 0.147502, 0.2)
        elif self.exc_axis == 'z':
            color = (0.471938, 0.530946, 0.8, 0.2)
        radius = 1
        steps = 360
    ...
    def modal(self, context, event):
        ...
        if event.type == 'LEFTMOUSE' or self.LB:
            ...
            if event.value == 'RELEASE':
                ...
                context.window.scene.transform_orientation_slots[0].type = self.user_orientation
...
def register():
    ...
    register_class(AdvancedScale)
    register_class(AdvancedRotation)
    register_class(SetConstarin)
...
def unregister():
    unregister_class(SetConstarin)
    unregister_class(AdvancedRotation)
    unregister_class(AdvancedScale)
    unregister_class(AdvancedMove)
